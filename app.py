...
def generate_preview():
    return generate_iframe(flask.request.args['url'])
...
def generate_iframe(url):
    ...
    if parsed.netloc.lower().endswith('youtube.com'):
        ...
        if 'list' in qs:
            return f'''<iframe width="100%" height="315" src="https://www.youtube.com/embed/videoseries?list={qs['list'][0]}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'''
        if 'v' in qs:
            return f'''<iframe width="100%" height="315" src="https://www.youtube.com/embed/{qs['v'][0]}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'''
        raise http_error.BadRequest("Missing playlist or video ID")

    req = requests.get(parsed.geturl())
    if req.status_code != 200:
        flask.abort(req.code)
    ...
    vidurl = find_opengraph('og:video:secure_url', 'og:video', 'twitter:player')
    if vidurl:
        width = find_opengraph('og:video:width', 'twitter:player:width') or '100%'
        height = find_opengraph('og:video:height', 'twitter:player:height') or '150'
        return f'<iframe src="{vidurl}" width="{width}" height="{height}" allow="accelerometer; autoplay; picture-in-picture"></iframe>'

    raise http_error.UnsupportedMediaType(f"Don't know how to handle URL {url}")
...
def submit_entry():
    ...
    output['Title'] = form['author-name']
    output['Author'] = form['author-name']
    if 'author-url' in form:
        output['Author-URL'] = form['author-url']

    output['submitter'] = user.identity

    if 'admin' not in user.groups:
        output['auth'] = user.identity
    ...
    if year < 2016:
        ...
        date = now.replace(year=year)
    elif year > now.year:
        raise http_error.BadRequest(f'{year} is in the future')
    output['Date'] = date.format('YYYY-MM-DD')

    output.set_content(get_entry_text(form['entry-url', form.get('comment')]))

    authorname = slugify.slugify(form['author-name'])
    filename = os.path.join('content', 'works', f'submission-{year} {authorname}.md')
    ...
    for fixup in range(5):
        if entry.scan_file(fullpath, filename, fixup):
            break
